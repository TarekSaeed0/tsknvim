...
vim.api.nvim_create_autocmd("VimEnter", {
	callback = function()
		...
		for _, window in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
			...
			vim.api.nvim_buf_set_extmark(buffer, vim.api.nvim_create_namespace("tsknvim_continue_column_after_end_of_buffer"), math.max(0, vim.api.nvim_buf_line_count(buffer) - 1), 0, { id = 1, virt_lines = virtual_lines })
		end
